...
# Weekly Awards: an optional, comedic "Week N Awards" section generated by the
# Claude API from that week's real results (scores, margins, bench points left
# unused, luck index, etc.) -- never invented stats, just roasted ones. Off by
# default (--weekly-awards + ANTHROPIC_API_KEY); if either is missing this whole
# section is skipped, same as SMS/email when their secrets aren't configured.
...
Projected: 2-12
Congratulations. Out of 14 teams, you convinced the projections that two wins
is your ceiling. That's not a fantasy team, that's a 12-week cry for help.

🍆 THE "BIGGEST DRAFT BONER" AWARD
Winner: Bubi's Giants
Projected: 12-2, #1 overall
Bubi didn't just draft a good team, he drafted a team and immediately started
mentally spending the league money. Please let this man go 1-5.

🗑️ THE "BIGGEST TRASH BAG" AWARD
Winner: Julienne Peppers
Projected: 14th
Looked at the draft board and thought "fuck rankings, fuck projections." And
honestly? Respect. Finishing dead last before Week 1 even starts takes commitment.
"""
...
def build_weekly_stats_packet(
    ...
    """Formats this week's real results into a plain-text block fed to the weekly
    awards prompt -- every number here comes straight from Sleeper, so the model
    has real material to roast instead of inventing stats."""
    lines = [f"Week {week} results:"]

    lines.append("\nMatchup results (winner, loser, scores, margin):")
    for m in matchups:
        if m.is_bye:
            t = m.teams[0]
        ...
        lines.append(
            f"- {winner['team']} {winner['points']:.2f} def. {loser['team']} {loser['points']:.2f} "
    ...
    for m in matchups:
        ...
        for t in m.teams:
            bench_pts = sum(
                pts
                for pid, pts in (t["players_points"] or {}).items()
                if pid not in t["starters"] and pts
            )
            if bench_pts > 0:
                bench_lines.append((t["team"], round(bench_pts, 2)))
    if bench_lines:
        lines.append("\nBench points left unused on the bench this week:")
        for team_name, bench_pts in sorted(bench_lines, key=lambda x: x[1], reverse=True):
            lines.append(f"- {team_name}: {bench_pts:.2f} points left on the bench")

    if closest_games:
        lines.append("\nClosest games this week:")
        for m in closest_games:
            winner, loser = m.winner, m.loser
            lines.append(
                f"- {winner['team']} beat {loser['team']} by {m.margin:.2f} "
                f"({winner['points']:.2f}-{loser['points']:.2f})"
            )

    if top_scorers:
        lines.append("\nTop individual scorers this week:")
        for s in top_scorers:
            lines.append(f"- {s['player']}: {s['points']:.2f} pts ({s['team']})")

    lines.append("\nCurrent standings (record, points for, points against):")
    for t in standings:
        lines.append(f"- {t.team_name}: {t.record}, {t.fpts:.2f} PF, {t.fpts_against:.2f} PA")

    if luck_index:
        lines.append(
            "\nLuck index (all-play record vs. actual record; positive = lucky, negative = unlucky):"
        )
        for entry in luck_index:
            lines.append(
                f"- {entry['team']}: {entry['record']} actual, {entry['all_play_record']} all-play, "
                f"luck {entry['luck_delta']:+.1f}"
            )

    if waivers:
        lines.append("\nWaiver/free-agent moves this week:")
        for w in waivers:
            added = ", ".join(w["added"]) or "—"
            dropped = ", ".join(w["dropped"]) or "—"
            lines.append(f"- {w['team']} added {added}, dropped {dropped}")

    return "\n".join(lines)


def generate_weekly_awards(
    stats_packet: str, league_name: str, week: int, *, api_key: str, model: str = DEFAULT_AWARDS_MODEL
) -> Optional[str]:
    """Calls the Claude API to write a set of comedic, R-rated 'weekly awards' for
    the league from this week's real results. Never raises -- returns None on any
    failure (network, auth, rate limit, unexpected response shape), so a broken or
    missing API key just skips this section rather than breaking the whole send."""
    system_prompt = (
        "You write savage, foul-mouthed, R-rated fantasy football \"weekly awards\" "
        "for a private home league newsletter. Every reader is a consenting adult "
        "friend in the league who expects to get roasted by name. Match this voice "
        "exactly -- profanity, sexual innuendo, direct \"you\" address, short punchy "
        "sentences, dry sarcasm -- here is a sample of the house style from a "
        "similar league's draft-day awards:\n\n" + AWARDS_STYLE_SAMPLE
    )
    user_prompt = (
    ...
    try:
        ...
        print(f"Skipping weekly awards: Claude API request failed ({exc})", file=sys.stderr)
        return None
    ...
    try:
        ...
        print("Skipping weekly awards: unexpected response shape from Claude API", file=sys.stderr)
        return None
    ...
    *,
    ...
    if weekly_awards_api_key and games_started:
        ...
        weekly_awards = generate_weekly_awards(
            stats_packet,
            league.get("name", "Fantasy League"),
            week,
            api_key=weekly_awards_api_key,
            model=weekly_awards_model,
        )
...
def render_markdown(data: NewsletterData) -> str:
    ...
    if data.weekly_awards:
        lines.append(f"## 🏆 Week {data.week} Awards\n")
...
.league-logo { display: block; max-width: 200px; margin: 0 auto 1rem; }
    ...
    if data.weekly_awards:
        parts.append(f"<h2>🏆 Week {data.week} Awards</h2>")
        for paragraph in data.weekly_awards.split("\n\n"):
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Generate a comedic 'Week N Awards' section via the Claude API from this "
            "week's real results (needs ANTHROPIC_API_KEY; see README). Skipped with "
            "a message if that's not set, same as email/SMS when their secrets are missing."
        ),
    )
    parser.add_argument(
        "--awards-model",
    ...
    weekly_awards_api_key = None
    if args.weekly_awards:
        weekly_awards_api_key = os.environ.get("ANTHROPIC_API_KEY")
        if not weekly_awards_api_key:
            print(
                "Skipping weekly awards: --weekly-awards passed but ANTHROPIC_API_KEY not set",
                file=sys.stderr,
            )
